...
i64 n, pie[51], suffix[52], dp[2][51]; // Max slices out of remaining that can be taken by ith player if they are on jth slice

i64 solve(i64 player, i64 pos) {
	if(pos == n) return 0;
	if(dp[player][pos]) return dp[player][pos];
	i64 n1 = pie[pos] + (suffix[pos+1] - solve(1-player, pos+1));
	i64 n2 = solve(player, pos+1);
	dp[player][pos] = Max(n1, n2);
	//cout << "dp[" << player << "][" << pos << "] = " << dp[player][pos] << endl;
...
int main() {
	...
	std::cin.tie(NULL);
	std::cout.tie(NULL);
	i64 t=0, m=0, maxx=0, minn=0, curr=0, k=0, num=0, siz=0, n1=0, n2=0, n3=0, n4=0, ind=0;
	...
	loop(i, n) cin >> pie[i];
	suffix[n] = 0;
	rloop(i, n) suffix[i] = suffix[i+1] + pie[i];
	q = solve(1, 0);
	cout << (suffix[0] - q) << ' ' << q;
