...
i64 a[300300], b[300300], up[300300], down[300300];
...
int main() {
	...
	loop(i, n) n1 += (a[i]*i);
	loop(i, n) n1 += (b[i]*(2*n-1-i));

	loop(i, n) n2 += (b[i]*(i+1));
	loop1(i, n-1) n2 += (a[i]*(2*n-i));
	...
	up[n] = 0;
	down[n] = 0;
	rloop(i, n) up[i] = (up[i+1] + a[i]);
	rloop(i, n) down[i] = (down[i+1] + b[i]);
	...
	loop1(i, n-1) {
		if(i%2) {
			...
			ans = max(ans, n1+x1);
			//cout << "n1 - " << n1+x1 << " - " << down[i+1] << " - " << up[i+1] << endl;
		}
		else {
			n2 -= (a[i]*(2*n-i));
			n2 -= (a[i-1]*(2*n-i+1));
			...
			ans = max(ans, n2+x2);
			//cout << "n2 - " << n2+x2 << " - " << down[i+1] << " - " << up[i+1] << endl;
		}
	}
