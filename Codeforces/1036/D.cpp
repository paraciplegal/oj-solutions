...
void nCkInit(ui64 m=MOD) {i64 i,mx=1010;fact.resize(mx+1);invfact.resize(mx+1);fact[0]=1;for(i=1;i<=mx;++i){
...
i64 get(string s) {
	i64 n = s.length(), ans = 0;
	loop1(len, n-1) {
		if(len > 2) ans += (9*(((len-1)*(len-2))/2)*9*9);
		if(len > 1) ans += (9*(len-1)*9);
		ans += 9;
	}
	//cout << "1-> " << ans << endl;
	i64 f = (i64)(s[0] - '0');
	loop1(i, f-1) {
		if(n > 2) ans += ((((n-1)*(n-2))/2)*9*9);
		if(n > 1) ans += ((n-1)*9);
		ans += 1;
	}
	//cout << "2-> " << ans << endl;
	...
	loop1(i, n-1) {
		i64 x = (i64)(s[i] - '0');
		bool flag = false;
		for(i64 k = 1; k < i; ++k) if(s[k] != '0') flag = true;
		loop1(j, n-1) {
			if(i < j) {
				bool flag2 = false;
				for(i64 k = i+1; k < j; ++k) if(s[k] != '0') flag2 = true;
				i64 y = (i64)(s[j] - '0');
				if(flag) {
					ans += (9*9);
				}
...
int main() {
	...
	while(t--) {
		cin >> s1 >> s2;
		cout << get(s2) - get(s1) + check(s1) << '\n';
		//cout << get(s2) << ' ' << get(s1) << ' ' << check(s1) << endl;
		/*stringstream s;
		n3 = 0;
		loop1(i, n1-1) {
			s << i;
