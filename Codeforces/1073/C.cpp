...
i64 l[200200], r[200200], u[200200], d[200200], ox, oy;

inline bool check(string &s, i64 x, i64 y, i64 k) {
	i64 ls = l[k], rs = r[k], us = u[k], ds = d[k], n = s.length();
	loop(i, n - k + 1) {
		bool flag = true;

		i64 dx = ox - rs + ls;
		i64 dy = oy - us + ds;
		if(abs(dx - x) + abs(dy - y) > k) flag = false;
		if((abs(dx - x) + abs(dy - y) - k)%2 != 0) flag = false;
...
int main() {
	...
	l[0] = r[0] = u[0] = d[0] = 0;
	loop(i, n) l[i+1] = (l[i] + (str[i] == 'L' ? 1 : 0));
	loop(i, n) r[i+1] = (r[i] + (str[i] == 'R' ? 1 : 0));
	loop(i, n) u[i+1] = (u[i] + (str[i] == 'U' ? 1 : 0));
	loop(i, n) d[i+1] = (d[i] + (str[i] == 'D' ? 1 : 0));

	lo = 1;
	hi = n;
	while(lo < hi) {
		mid = (lo+hi)/2;
		//if(check(str, x, y, mid)) cerr << "----- " << mid << endl;
		if(check(str, x, y, mid)) hi = mid;
		else lo = mid + 1;
	}
	cout << lo;
