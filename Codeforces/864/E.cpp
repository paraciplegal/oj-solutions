...
i64 t[102], d[102], p[102], srt[102], dp[2020][102], taken[2020][102];
stack<i64> sol;

void trace(i64 tim, i64 pos) {
	//cout << tim << ' ' << pos << ' ' << taken[tim][pos] << endl;
	if(taken[tim][pos] > 0) {
		sol.push(taken[tim][pos]-1);
		trace(tim-t[srt[taken[tim][pos]-1]], taken[tim][pos]-1);
	}
}
...
int main() {
	...
	std::cin.tie(NULL);
	std::cout.tie(NULL);
	i64 n=0, m=0, maxx=0, minn=0, curr=0, k=0, num=0, siz=0, n1=0, n2=0, n3=0, n4=0, ind=0;
	i64 root=0, sum=0, diff=0, q=0, choice=0, len=0, begg=0, endd=0, pos=0, cnt=0, lo=0, hi=0, mid=0, ans=0;
	bool flag = false;
	...
	cin >> n;
	loop(i, n) cin >> t[i] >> d[i] >> p[i];
	loop(i, n) srt[i] = i;
	sort(srt, srt+n, [&](i64 x, i64 y){return d[x] < d[y];});
	loop(i, 2005) {
		loop1(j, n) {
			if(i == 0) dp[i][j] = 0;
			else {
			...
			if(i >= t[srt[j-1]] && i < d[srt[j-1]]) {
				i64 x = dp[i-t[srt[j-1]]][j-1]+p[srt[j-1]];
				if(x > dp[i][j]) {
					dp[i][j] = x;
					taken[i][j] = j;
				}
